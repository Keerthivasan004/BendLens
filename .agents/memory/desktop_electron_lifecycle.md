...
- **Re-download over an existing install**: `BendLens-Setup.cmd` detects `%LOCALAPPDATA%\BendLens\package.json`, kills running BendLens processes first (avoids locked/half-written files), then updates in place. NSIS one-click reinstalls over the old version while `customInstall` clears legacy/stale caches. A launch-while-running duplicate just focuses the existing window (mutex + single-instance lock); a version-mismatched live engine is never reused.
- **Full removal**: NSIS Uninstall (with `deleteAppDataOnUninstall` + `customUnInstall`) wipes app + `%APPDATA%\BendLens`/`com.bendlens.studio`, legacy payload dir, temp history, and shortcuts. Legacy-payload users get `Uninstall-BendLens.cmd` — generated by `scripts/build-installer.js` into `public/downloads/` and written into `%LOCALAPPDATA%\BendLens\` at install time — which performs the same full wipe.
